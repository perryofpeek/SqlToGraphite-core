namespace Plugin.Oracle.Transactions
{
    public class Sql
    {
        ...
        public const string GetTransactionsCountSql =
            "select originating_system, count(1) from transactions where current_ts_code = 'FIRM' and id > {0} and to_char(start_date_time,'HH24:MI:SS') > to_char(sysdate-{1}/(24*60*60),'HH24:MI:SS') group by originating_system";
    }
}