namespace Plugin.Oracle.Transactions
{
    public class Sql
    {
        ...
        public const string GetTransactionsCountSql = "select originating_system, count(1) from transactions where current_ts_code = 'FIRM' and id > {0} and start_date_time > sysdate-{1}/(24*60*60) group by originating_system";    
    }
}